...
window.customElements.define('tm-sites', class extends LitElement {
    ...
    constructor() {
        ...
        this.sites = {
            'site': 'https://wonkytech.net',
            'src': 'https://github.com/tmcmaster',
            'pika': 'https://www.pika.dev/search?q=@wonkytech',
            'npm': 'https://www.npmjs.com/~tmcmaster',
            'docs': 'http://tim.mcmaster.id.au'
        };
    }
    ...
    render() {
        ...
        return html`
            ${Object.keys(this.sites).map(key => html`
                <button placeholder="First name" @click="${() => this.openSite(key)}" class="icon ${key}"></button>
            `)}
        `;
    }
});
